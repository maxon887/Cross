...
vec3 CalcPointLight(PointLight light, vec3 diffuseColor, vec3 specularColor, float shininess){
...
vec3 CalcSpotLight(SpotLight light, vec3 diffuseColor, vec3 specularColor, float shininess){
	vec3 lightDirection = normalize(light.position - vFragPosition);
...
void main(){
	...
	vec3 specularColor = vec3(texture2D(uSpecularMap, vTexCoords)) * 4.0;
	float shininess = vec3(texture2D(uShininessMap, vTexCoords)).r * 128.0;
